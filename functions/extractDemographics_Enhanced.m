...
participantDir = fullfile('E:\Dropbox\LeuchterGrandjean_lc_sg\data', participant, 'logfiles');

% Look for the Resultfile with pattern matching
logFiles = dir(fullfile(participantDir, '*Voice__P*Block1.txt'));
...
logFilePath = fullfile(logFiles(1).folder, logFiles(1).name);
...
try
    ...
    if size(logData, 1) >= 2 && size(logData, 2) >= 1
        ...
        if isnumeric(genderCell) && genderCell == 1
            gender = 'female';
        elseif isnumeric(genderCell) && genderCell == 2
            gender = 'male';
        else
            gender = 'Unknown';
        end
    ...
    disp(ME.message);  % ← ADD THIS
    disp(ME.stack(1)); % ← AND THIS - shows exactly which line failed
    gender = 'Unknown';
    age = NaN;
end
end
