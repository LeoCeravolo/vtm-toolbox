function [conditionData, conditionLabels, outlierInfo] = extractConditionDataWithOutlierRemoval_Direct(conditionDataStructure, conditions, metricName)
% EXTRACTCONDITIONDATAWITHOUTLIERREMOVAL_DIRECT - Adapted from your original
% Extract condition data and remove outliers using 3-sigma rule (same logic as your original)
...
% First pass: collect all data to establish global statistics (same as your original)
allData = [];
allConditionLabels = [];
...
for condIdx = 1:length(conditions)
    ...
    if isfield(conditionDataStructure, condName) && ...
            isfield(conditionDataStructure.(condName), metricName) && ...
            ~isempty(conditionDataStructure.(condName).(metricName))

        data = conditionDataStructure.(condName).(metricName);
...
% Calculate global statistics for outlier detection (same as your original)
...
% Second pass: remove outliers from each condition (same as your original)
for condIdx = 1:length(conditions)
    condName = conditions{condIdx};
    if isfield(conditionDataStructure, condName) && ...
            isfield(conditionDataStructure.(condName), metricName) && ...
        ...
        if ~isempty(validData)
            ...
            % Log outlier removal (same as your original)
